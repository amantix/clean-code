...
public class ChangeAccessFilter(IDocumentAccessRepository documentAccessRepository): IAsyncActionFilter
{
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        if (context.ActionArguments.FirstOrDefault().Value is not ChangeAccessLevelRequest parameter)
        {
        ...
        {
            context.Result = new BadRequestObjectResult("You do not have permission to change access of this document.");
            return;
        }
        await next();
    }
