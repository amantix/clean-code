...
public class DeleteAccessFilter(IDocumentAccessRepository documentAccessRepository): IAsyncActionFilter
{
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        if (context.ActionArguments.FirstOrDefault().Value is not DeleteDocumentAccessRequest parameter)
        {
        ...
        {
            context.Result = new BadRequestObjectResult("You do not have permission to delete this document.");
            return;
        }
        await next();
    }
