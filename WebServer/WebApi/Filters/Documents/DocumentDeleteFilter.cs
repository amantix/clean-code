...
public class DocumentDeleteFilter(IAccessService accessService) : IAsyncActionFilter
{
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        if (context.ActionArguments.TryGetValue("request", out var argument))
        {
            ...
            if (!checkMasterAccessResult.IsSuccess)
                context.Result = new BadRequestObjectResult("Недостаточно прав для удаления документа!");
        }
